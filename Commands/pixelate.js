...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'pixelate', [], (args, message) => {
            ...
            if (args.length < 2){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: \n`' + rand.commandPrefix + 'pixelate (url | @user) (percent)`');
            ...
            if (isNaN(args[1])){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", '**Invalid percentage.**\n**Usage**: *`' + rand.commandPrefix + 'pixelate (url | @user) (percent)`*');
                    ...
                    if (!err) {
                        ...
                        download.image(options).then(({ filename }) => {
                            Jimp.read(filename).then(img => {
                                img.pixelate(contrast).quality(100).write(__dirname + "/../Images/" + id + ".jpg");
