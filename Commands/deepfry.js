...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'deep-fry', [], (args, message) => {
            ...
            if (args.length < 1){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: `' + rand.commandPrefix + 'deepfry (url | @user)`');
                    ...
                    if (!err) {
                        ...
                        sightengine.check(['face-attributes']).set_url(URL).then(function(result2) {
                            ...
                            download.image(options).then(({ filename }) => {
                                    ...
                                    let emoji = images(__dirname + "/../Images/bbe3.png");
                                    let ok = images(__dirname + "/../Images/ok.png");
                                    let found = false;
                                    ...
                                    result2.faces.forEach(k => {
                                        ...
                                        ok.size(width / 2, height / 2);

                                        img.draw(emoji, x1, y1);
                                        img.draw(ok, result.width * 0.75, result.height * 0.25);
                                    ...
                                    if (!found){
                                        ...
                                        ok.size(result.width / 2, result.height / 2);
                                        img.draw(emoji, result.width / 4, result.height / 4);
                                        img.draw(ok, result.width / 2, result.height / 2);
                                    }
                                    ...
                                    Jimp.read(__dirname + "/../Images/" + id + ".jpg").then(img => {
                                        img.brightness(0.2).contrast(0.9).quality(100).write(__dirname + "/../Images/" + id + ".jpg");
                                        ...
                                        imgur.uploadFile(__dirname + "/../Images/" + id + ".jpg").then(function (json) {
                                            ...
                                            embed
                                                .setColor(0xFF7F00)
                                                .addField("**__IMAGE ERROR__**", 'Something broke while frying your image.\nThis is probably on my side.');
                                            m.edit({embed});
                                        ...
                                        embed
                                            .setColor(0xFF7F00)
                                            .addField("**__IMAGE ERROR__**", 'Your image got way too fried.');
                                        m.edit({embed});
                                        console.error(err);
                                        delete rand.process[gId];
                                    });
                            ...
                            embed
                                .setColor(0xFF7F00)
                                .addField("**__IMAGE ERROR__**", 'Your image is immune to fry-ers.');
                            m.edit({embed});
                            console.log(err);
