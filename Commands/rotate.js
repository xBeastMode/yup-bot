...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'rotate', [], (args, message) => {
            ...
            if (args.length < 2){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: \n`' + rand.commandPrefix + 'rotate (url | @user) (degrees)`');
            ...
            if (isNaN(args[1])){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", '**Invalid degrees.**\n**Usage**: *`' + rand.commandPrefix + 'rotate (url | @user) (degrees)`*');

                message.channel.send({embed});
                return;
            }

            if (Number(args[1]) > 360 || Number(args[1]) < 1){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", '**Invalid degrees.**\n*Degrees must go from 1-360.*');
                message.channel.send({embed});
                return;
            }
                    ...
                    if (!err) {
                        ...
                        download.image(options).then(({ filename }) => {
                            Jimp.read(filename).then(img => {
                                img.rotate(width).quality(100).write(__dirname + "/../Images/" + id + ".jpg");
                                ...
                                embed
                                    .setColor(0xFF7F00)
                                    .addField("**__IMAGE ERROR__**", 'Error while rotating image.');
                                m.edit({embed});
                                console.error(err);
                                delete rand.process[gId];
                            });
