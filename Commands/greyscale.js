...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'greyscale', [], (args, message) => {
            ...
            if (args.length < 1){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: \n`' + rand.commandPrefix + 'greyscale (url | @user)`');
                    ...
                    if (!err) {
                        ...
                        download.image(options).then(({ filename }) => {
                            Jimp.read(filename).then(img => {
                                img.greyscale().quality(100).write(__dirname + "/../Images/" + id + ".jpg");
