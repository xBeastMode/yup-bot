...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'brightness', [], (args, message) => {
            ...
            if (args.length < 2){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: \n`' + rand.commandPrefix + 'brightness (url | @user) (percent)`');
            ...
            if (isNaN(args[1])){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", '**Invalid percentage.**\n**Usage**: *`' + rand.commandPrefix + 'brightness (url | @user) (percent)`*');
            ...
            let URL = args[0];
            let contrast = Number(args[1]) / 100;
                    ...
                    if (!err) {
                        ...
                        download.image(options).then(({ filename }) => {
                            Jimp.read(filename).then(img => {
                                img.brightness(contrast).quality(100).write(__dirname + "/../Images/" + id + ".jpg");
