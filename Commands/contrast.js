...
module.exports = {
    init: function () {
        ...
        rand.registerCommand(rand.commandPrefix, 'contrast', [], (args, message) => {
            ...
            if (args.length < 2){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", 'Usage: \n`' + rand.commandPrefix + 'contrast (url | @user) (percent)`');
            ...
            if (isNaN(args[1])){
                embed
                    .setColor(0xFF0000)
                    .addField("**__ERROR__**", '**Invalid percentage.**\n**Usage**: *`' + rand.commandPrefix + 'contrast (url | @user) (percent)`*');
                    ...
                    if (!err) {
                        ...
                        download.image(options).then(({ filename }) => {
                            Jimp.read(filename).then(img => {
                                img.contrast(contrast).write(__dirname + "/../Images/" + id + ".jpg");
